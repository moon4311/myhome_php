...
// 오늘의 횟수를 체크
$sql = " select count(*) as cnt from $taja_table where mb_id = '$member[mb_id]' and taja_datetime like '$g4[time_ymd]%' ";
$result = sql_fetch($sql);
if ($result['cnt'] > $today_max) {
    alert("오늘의 횟수 $today_max 를 초과하셨습니다. 과도한 게임은 서버의 건강에 좋지 않습니다.");
}
?>
<div <?=isset($g4['body_script']) ? $g4['body_script'] : "";?> <? if (!$member[mb_level] < 9) { ?>oncontextmenu='return false' onderagstart='return false' onselectstart='return false'<? } ?>>
<link type="text/css" rel="stylesheet" media="all" href="css/style.css" />
	<div class="top-summary">
	<p></p>
	<h2 style="center">Get a Point 타자게임</h2>
	<div class="content">
		<ul>
			<li>타 프로그램의 타자속도 및 통상적인 타자속도와 다르므로 착오없으시기 바랍니다.</li>
			<li>과도한 게임은 건강에 좋지 않습니다. 건강을 위해 하루 <?=$today_max?> 회로 제한 됩니다.</li>
			<li>타자 정확도의 백분율에 따라 포인트가 차감 됩니다 (예: 80% 정확도*10포인트= 8포인트지급)</li>
			<li>1번 게임에 <?=$p1?> 포인트, 본인의 기록을 갱신하면 <?=$p2?> 포인트를 지급 합니다.</li>
			<li>비회원은 포인트가 지급되지 않으며, 비정상적 타수(<?=$max_speed?> 타 이상)는 기본 포인트만 지급 됩니다.</li>
      <?
	    ...
	    $max_nick = $mb['mb_nick'];
      ?>
    	<li><?=$config[cf_title]?> 최고기록은 <b><?=$max_nick?></b>님의 <b><?=number_format($max_taja)?> 타</b> 입니다.</li>
      <? 
      if ($member[mb_id]) { 
      ...
      ?>
    			<li><?=$member[mb_nick]?>님의 최고기록은 <?=number_format($max_mb_taja)?> 타 입니다.</li>
			<? } ?>
			</ul>
	</div>
</div>
...
//<![CDATA[
		...
		function Http(){
          ...
          $.ajax({
            type: 'POST',
            url: g4_path+'/plugin/taja2/ajax_post.php',
            data: {
                'mb_id': encodeURIComponent('<?=$member[mb_id]?>'),
