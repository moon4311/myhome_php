...
$img_width = 270; // 이미지 가로 사이즈
$img_height = 160; // 이미지 세로 사이즈

$frame_width = 260; // 가로길이
...
div#acroller {position:relative;height:160px; width:<?=$frame_width?>;}

/*좌우버튼*/
ul#acrollerFrame {width:232px; padding:0;margin:0;list-style:none;}
ul#acrollerFrame li {position:relative;float:left; margin:0 0 0 0px;width:232px;height:160px;}


/* new */

.price {font-family:돋움;font-size:12px;text-align:center;color:#000000}
.besticon {position:absolute;top:0;left:-1;}
							...
							if(!$gdversion){ 
								...
								if($img_width > $get_img[0] || $img_height > $get_img[1]){
									...
									createThumb_list($img_width,$img_height,$src, $thumb_file_list, $get_img); // list 페이지 썸네일
                                                ...
                                                echo '<img class="besticon" src="skin/latest/ok-slider0/img/new.png">';
						$img = "<a href='{$list[$i]['href']}'><img src=\"$thumb_file_list\" border=\"1\" width=\"$img_width\" height=\"$img_height\"/ >";



					}else{
						// no 이미지를 비율적으로 만들어났음
						$img = "<a class=price' href='{$list[$i]['href']}'><img src=\"$latest_skin_path/img/no_image.gif\" border=\"1\" height=\"{$img_height}\">";
					}echo $img;
                                                 echo '</div>';
			?>
		</li>
